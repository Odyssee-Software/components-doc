...
const mountPulseButton = async () => {
  ...
  try {
    ...
    const { Button } = await import('@odyssee/components');
...
onMounted(() => {
  ...
  const slot = containerRef.value?.parentElement?.querySelector('.button-text');
  if (slot) {
...
watch(() => [props.variant, props.color, props.size, props.disabled, props.loading, props.icon], () => {
  mountPulseButton();
}, { deep: true });
